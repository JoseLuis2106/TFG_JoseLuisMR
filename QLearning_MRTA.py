...
class QLearning:
    ...
    def choose_act(self,state,tasks_states):
        """
        Elige la acción correspondiente mediante epsilon-greedy. Distingue entre espacio de acciones discreto o multidiscreto.
        """
        valid_actions = [a for a in self.valid_act_space if self._validate_action(a, tasks_states)]
    ...
    def _validate_action(self,action,tasks_states):
        """
        Comprueba si alguna de las tareas asignadas ya estan completadas o falladas
        """
        action = np.asarray(action) 
        for task, robot in enumerate(action):
            if robot > 0 and tasks_states[task] != 0:       # Si la tarea no está pendiente y se le ha asignado un robot
